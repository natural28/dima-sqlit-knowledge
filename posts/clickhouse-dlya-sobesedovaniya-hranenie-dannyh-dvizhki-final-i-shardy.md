...
| Движок | Когда нужен | Простое объяснение |
|---|---|---|
| `MergeTree` | обычная большая таблица | хранит данные кусками и сливает их в фоне |
| `ReplacingMergeTree` | есть несколько версий одной сущности | со временем оставляет одну актуальную версию |
| `SummingMergeTree` | нужно заранее суммировать числовые показатели | при слиянии складывает числовые колонки по ключу |
| `AggregatingMergeTree` | нужны более сложные предрасчёты | хранит специальные состояния агрегатных функций |
| `ReplicatedMergeTree` | нужна копия данных на другом сервере | обычный `MergeTree`, но с репликацией |
| `ReplicatedReplacingMergeTree` | нужны версии строк и репликация | `ReplacingMergeTree`, но с копиями данных на репликах |
